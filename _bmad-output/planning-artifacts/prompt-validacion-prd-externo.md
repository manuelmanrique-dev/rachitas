# Prompt: Validación de PRD

Copia todo el contenido de este archivo y pégalo en otro modelo de IA (GPT-4o, Gemini, DeepSeek, etc.) para obtener una validación independiente del PRD.

---

## INSTRUCCIONES

Eres un auditor senior de documentos de producto. Tu tarea es validar rigurosamente el PRD (Product Requirements Document) que se incluye al final de este prompt. Responde siempre en español.

Ejecuta las siguientes 13 validaciones en orden. Para cada una, indica el resultado como **PASS**, **WARNING** o **CRITICAL**, con explicación concreta y ejemplos específicos del documento.

---

### Validación 1: Estructura del Documento

Verifica que el PRD contiene estas 6 secciones core:
1. Executive Summary (o variaciones: Overview, Introduction)
2. Success Criteria (o: Goals, Objectives)
3. Product Scope (o: Scope, In Scope, Out of Scope)
4. User Journeys (o: User Stories, User Flows)
5. Functional Requirements (o: Features, Capabilities)
6. Non-Functional Requirements (o: NFRs, Quality Attributes)

Clasifica: **Standard** (5-6 presentes), **Variant** (3-4), **Non-Standard** (<3).

---

### Validación 2: Densidad de Información

Escanea el documento buscando estos anti-patrones:

**Relleno conversacional:**
- "The system will allow users to..." → debería ser "Users can..."
- "It is important to note that..." → decir el hecho directamente
- "In order to..." → "To..."
...
**Severidad:** CRITICAL (>10 violaciones), WARNING (5-10), PASS (<5).

---

### Validación 3: Cobertura del Product Brief

El PRD debería cubrir estos elementos del brief del producto:
- Visión del producto
...
Para cada uno, clasifica: **Fully Covered**, **Partially Covered**, **Not Found**, **Intentionally Excluded**.
...
- ¿Siguen formato "[Actor] puede [capacidad]"?
- ¿Contienen adjetivos subjetivos? (fácil, rápido, simple, intuitivo)
- ¿Contienen cuantificadores vagos? (múltiples, varios, algunos, muchos)
- ¿Contienen detalles de implementación? (tecnologías, librerías)

**Requisitos No Funcionales (NFRs):**
- ¿Tienen métricas específicas?
- ¿Tienen método de medición?
- ¿Tienen contexto?
...
Valida la cadena:
```
Visión → Success Criteria → User Journeys → Functional Requirements
```

Busca:
- FRs que no se trazcan a ningún user journey
- Success criteria sin journeys que los soporten
- User journeys sin FRs que los implementen

**Severidad:** CRITICAL (FRs huérfanos), WARNING (gaps), PASS (trazabilidad intacta).

---

### Validación 6: Fuga de Implementación

Escanea FRs y NFRs buscando menciones de tecnologías específicas:
...
- Librerías: Redux, Zustand, axios, lodash

**Nota importante:** Este PRD menciona tecnologías intencionalmente en secciones de contexto técnico (como "Technical Success" y "Project Classification"). La fuga de implementación solo es problemática cuando aparece DENTRO de los requisitos funcionales (FRs) y no funcionales (NFRs) — no en secciones descriptivas de contexto.

**Severidad:** CRITICAL (>5 en FRs/NFRs), WARNING (2-5 en FRs/NFRs), PASS (<2 en FRs/NFRs).
...
- **Dominio:** Wellness / Desarrollo Personal (complejidad baja)
- **Tipo:** Web App (PWA/SPA)

Para el dominio wellness (no médico), verifica que:
- No asume regulación médica innecesaria
- Incluye consideraciones de privacidad apropiadas (GDPR)
- Maneja datos sensibles (hábitos, conversaciones) con protección adecuada

---

### Validación 8: Compliance de Tipo de Proyecto (Web App)

Verifica que incluye las secciones requeridas para web app:
...
Verifica que NO incluye secciones irrelevantes (features nativas, comandos CLI).

---

### Validación 9: Validación SMART de Requisitos

Evalúa una muestra representativa de FRs (al menos 10) en escala 1-5 para cada dimensión:

- **S**pecific: Claro, sin ambigüedad
- **M**easurable: Cuantificable, testeable
- **A**ttainable: Realista dentro de las restricciones
- **R**elevant: Alineado con necesidades del usuario
- **T**raceable: Vinculado a journey o objetivo de negocio

Marca los FRs con score <3 en cualquier categoría.

**Severidad:** CRITICAL (>30% marcados), WARNING (10-30%), PASS (<10%).
...
- ¿Las transiciones entre secciones son lógicas?
- ¿Es consistente en tono y detalle?

**Efectividad para dos audiencias:**
- *Para humanos:* ¿Un ejecutivo entiende la visión rápido? ¿Un desarrollador puede construir desde esto?
- *Para LLMs:* ¿La estructura es machine-readable? ¿Se puede generar arquitectura, UX o epics desde este documento?

**Rating general (1-5):**
- 5: Ejemplar, listo para producción
- 4: Fuerte con mejoras menores
- 3: Adecuado pero necesita refinamiento
- 2: Gaps significativos
- 1: Problemas mayores
...
**Checks finales:**
- ¿Quedan variables de template? ({variable}, [placeholder])
- ¿Todas las secciones core están completas?
- ¿Todos los success criteria son medibles?
- ¿Los journeys cubren todos los tipos de usuario?
- ¿Los FRs cubren el scope del MVP?
- ¿Los NFRs tienen criterios específicos?

---

### Validación 12: Top 5 Problemas + Top 5 Fortalezas

Lista los 5 problemas más importantes y las 5 fortalezas más destacables del documento.
...
|-----------|-----------|-------|
| Estructura | PASS/WARNING/CRITICAL | ... |
| Densidad | PASS/WARNING/CRITICAL | ... |
| Cobertura Brief | PASS/WARNING/CRITICAL | ... |
| Medibilidad | PASS/WARNING/CRITICAL | ... |
| Trazabilidad | PASS/WARNING/CRITICAL | ... |
| Fuga Implementación | PASS/WARNING/CRITICAL | ... |
| Dominio | PASS/WARNING/CRITICAL | ... |
| Tipo Proyecto | PASS/WARNING/CRITICAL | ... |
| SMART | PASS/WARNING/CRITICAL | ... |
| Calidad Holística | X/5 | ... |
| Completitud | PASS/WARNING/CRITICAL | ... |

**Veredicto final:** ¿Está listo para pasar a diseño UX y arquitectura? ¿Qué se debe corregir antes?
...
**Opening Scene:** Martes por la noche. Marta está en Instagram y ve un post de alguien que comparte su racha de lectura con una estética que le llama la atención. Hace clic, llega a la web de Rachitas. Ve "Cuéntame, ¿qué quieres cambiar?" y algo le hace quedarse.
...
**Requirements revelados:** Onboarding conversacional, diseño de hábito guiado, heatmap con gradientes, sistema de cumbres (post-MVP), widget de registro rápido (post-MVP), resumen auto-generado desde conversación.
...
- FR7: El sistema elimina todos los datos del usuario de Supabase y Dexie.js al confirmar la eliminación
...
- FR11: El agente interactúa con los datos de la aplicación (hábitos, registros, objetivos) mediante un framework agéntico tipo MCP
- FR12: El agente puede crear, modificar y consultar hábitos del usuario como resultado de la conversación
- FR13: El agente mantiene un tono empático y nunca punitivo, codificado en sus prompts
- FR14: El agente advierte cuando detecta un hábito potencialmente poco realista y sugiere alternativas
- FR15: El agente da la bienvenida al usuario que vuelve tras una ausencia sin generar culpa
...
- FR34: El sistema sincroniza automáticamente los datos locales (Dexie.js) con el servidor (Supabase) al recuperar conexión
- FR35: El sistema mantiene Supabase como fuente de verdad para resolver conflictos de sincronización
...
- FR38: La aplicación funciona como app nativa (pantalla completa, splash screen, iconos)
- FR39: La aplicación carga y es funcional para registro de hábitos cuando no hay conexión

### Administración

- FR40: El administrador puede acceder a métricas básicas de uso a través de Supabase Dashboard
- FR41: El administrador puede gestionar usuarios (visualizar, banear) a través de Supabase Dashboard
...
- NFR5: La sincronización offline-to-online no bloquea la interacción del usuario

### Security

- NFR6: Todos los datos se transmiten cifrados (HTTPS/TLS en tránsito)
- NFR7: Los datos en Supabase están cifrados en reposo (cifrado nativo de Supabase/PostgreSQL)
...
- NFR10: Row Level Security (RLS) en Supabase garantiza que un usuario solo accede a sus propios datos
...
- NFR14: La arquitectura soporta hasta 1,000 usuarios activos mensuales sin cambios de infraestructura (Supabase free/Pro tier)
...
- NFR16: Dexie.js como cache local no degrada rendimiento con hasta 2 años de datos de hábitos por usuario
...
- NFR21: La interfaz conversacional es inherentemente accesible — el texto es el medio primario de interacción

### Integration

- NFR22: El framework agéntico abstrae la comunicación con modelos IA detrás de una interfaz uniforme, permitiendo cambiar de proveedor (open source self-hosted, OpenAI, Anthropic, etc.) sin modificar lógica de negocio
- NFR23: La integración con el modelo IA gestiona errores de red, timeouts y respuestas malformadas sin crashear la aplicación — el usuario recibe feedback claro si el agente no puede responder
...
- NFR26: Supabase Auth soporta al menos 2 proveedores de identidad (email + 1 social) con flujo unificado
- NFR27: La sincronización Dexie.js ↔ Supabase implementa cola de reintentos para operaciones fallidas, con máximo 3 reintentos y backoff exponencial
- NFR28: El sistema de integración con IA registra métricas de uso (tokens consumidos, latencia, errores) accesibles desde Supabase Dashboard para monitoreo de costes
- NFR29: Si el servicio de IA externo no está disponible, la app sigue siendo completamente funcional para registro y consulta de hábitos — la IA es una mejora, no una dependencia bloqueante
